...
until docker-compose exec mysql_slave_0 sh -c 'export MYSQL_PWD=111; mysql -u root -e ";"'
do
    echo "Waiting for mysql_slave_0 database connection..."
    sleep 4
done
...
docker exec mysql_slave_0 sh -c "$start_slave_cmd"
docker exec mysql_slave_0 sh -c "export MYSQL_PWD=111; mysql -u root -e 'SHOW SLAVE STATUS \G'"
